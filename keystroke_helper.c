//
// Created by alexey on 05.12.2020.
//
...
//    int len = ;
    if (write(STDOUT_FILENO, helper_message, (strlen(helper_message)+1)) == -1) {
...
//                    ev[i].time.tv_usec, evval[ev[i].value], (int) ev[i].code, (int) ev[i].code);
    ...
    if (write(STDOUT_FILENO, &num_actions, sizeof(num_actions)) == -1) {
        syslog(LOG_DEBUG, "helper: cannot send keys number from helper");
        retval = PAM_AUTH_ERR;
    }
    if (write(STDOUT_FILENO, array_of_actions, num_actions * sizeof(struct input_event)) == -1) {
        syslog(LOG_DEBUG, "helper: cannot send keys number from helper");
        retval = PAM_AUTH_ERR;
    }
    close(fd);
